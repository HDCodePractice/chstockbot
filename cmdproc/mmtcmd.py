...
def process_income_message(incoming_message, user):
    ...
    # 从本地查找股票代码是否存在
    file = search_file(msg_l[1].lower().replace(
        ".", "-") + ".us.txt", os.path.expanduser(config.config_path))
    if not file:
    ...
    one_year_date = datetime.date(
        mmt_endtime.year-1, mmt_endtime.month, mmt_endtime.day)
    ten_year_date = datetime.date(
        mmt_endtime.year-10, mmt_endtime.month, mmt_endtime.day)
    # 准备返回3个按钮； 传入profit_list里的key 和用户名
    keyboard = [[
        InlineKeyboardButton(
            text=f"{mmt_starttime}", callback_data=f"{msg_l[1]}:{mmt_starttime}:{mmt_endtime}:{user}"),
        InlineKeyboardButton(
            text=f"过去一年", callback_data=f"{msg_l[1]}:{one_year_date}:{mmt_endtime}:{user}"),
        InlineKeyboardButton(
            text=f"过去10年", callback_data=f"{msg_l[1]}:{ten_year_date}:{mmt_endtime}:{user}")
    ]]
...
def mmt_command(update: Update, context: CallbackContext) -> None:
    ...
    reply_message, keyboard = process_income_message(
        incoming_message.text, user)
    if keyboard == None:
        incoming_message.reply_text(reply_message)
        return
    reply_markup = InlineKeyboardMarkup(keyboard)
    context.bot.send_message(chat_id=incoming_message.chat_id, text=reply_message.replace(
...
def announce_mmt(update: Update, context: CallbackContext):  # 获取点击按钮的用户信息和毛毛投信息
    ...
    # 直接读取profit_list里的value
        # 处理毛毛投利润
    try:
        profit_msg = process_ticker_profit(
            mmt_data[0].lower(), mmt_data[1], mmt_data[2])
    ...
    update.callback_query.edit_message_text(
        text=f"亲爱的{reply_user_name},{profit_msg}", reply_markup=update.callback_query.message.reply_markup)
