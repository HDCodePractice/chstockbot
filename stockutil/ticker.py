...
from util.utils import (get_dmm_maxtry, get_target_date, get_xmm_maxtry,
                        is_second_wednesday)

from stockutil.stooq import (maNotEnoughError, markCloseError, read_stooq_file,
                             search_file)
...
class Ticker:
    ...
    def load_data(self):
        ...
        if self.ds != None:
            if self.from_s.lower() == "web":
                df = web.DataReader(
                    self.symbol.upper(), self.ds, start=self.starttime, end=self.endtime)
            ...
            if self.from_s.lower() == "local":
                tiker_file = search_file(self.symbol.lower().replace(
                    ".", "-") + ".us.txt", os.path.expanduser(self.ds))
                try:
    ...
    def cal_profit(self):
        ...
        if self.get_price_list():
            ...
            self.xmm_profit = {
                # 当前市值
                "current_price": xmm_stock_number * self.df["Close"][-1],
                # 总成本
                "total_principle": self.principle * len(self.xmm_price_list),
                # 盈利百分比
                "profit_percentage": (xmm_stock_number * self.df["Close"][-1] - self.principle * len(self.xmm_price_list))/(self.principle * len(self.xmm_price_list))
            }
            if len(self.dmm_price_list) > 0:
                self.dmm_profit = {
                    # 当前市值
                    "current_price": dmm_stock_number * self.df["Close"][-1],
                    # 总成本
                    "total_principle": self.principle * len(self.dmm_price_list),
                    # 盈利百分比
                    "profit_percentage": (dmm_stock_number * self.df["Close"][-1] - self.principle * len(self.dmm_price_list))/(self.principle * len(self.dmm_price_list))
                }
            return True
    ...
    def cal_today_price_rate(self):  # 计算今日与昨日的收盘价差异
        df = self.df
        percentage = (df['Adj Close'][-1] - df['Adj Close']
                      [-2])/df['Adj Close'][-2] * 100
    ...
    def gen_mmt_msg(self):
        chat_msg = ""
        if self.xmm_profit:
            if self.xmm_profit['profit_percentage'] > 0:
                ...
                chat_msg += f"从{self.starttime.strftime('%Y年%m月%d日')}定投 #小毛毛 {self.symbol}，到{self.endtime.strftime('%Y年%m月%d日')}累计投入 {self.xmm_profit['total_principle']}元，到昨日市值为 {self.xmm_profit['current_price']:0.2f} 元，利润为 {self.xmm_profit['profit_percentage']*100:0.2f}%\n"
        if self.dmm_profit:
            if self.dmm_profit['profit_percentage'] > 0:
