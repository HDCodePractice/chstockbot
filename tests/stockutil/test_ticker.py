...
def test_ticker_load_data_local(shared_datadir):
    """Test ticker load data."""
    ticker = Ticker("AAPL", "local", f"{shared_datadir}", starttime=date(
        2021, 1, 1), endtime=date(2021, 8, 20))
    df = ticker.load_data()
    ...
    ticker = Ticker("GOEV", "local", f"{shared_datadir}", starttime=date(
        2021, 1, 1), endtime=date(2021, 8, 20))
    df = ticker.load_data()
...
def test_ticker_load_data_local_error(shared_datadir):
    ...
    # axas到8月3日，20号没有交易
    ticker = Ticker("axas", "local", f"{shared_datadir}", starttime=date(
        2021, 1, 1), endtime=date(2021, 8, 20))
    ...
    # finw没有数据
    ticker = Ticker("finw", "local", f"{shared_datadir}", starttime=date(
        2021, 1, 1), endtime=date(2021, 8, 20))
    with pytest.raises(TickerError) as e:
        ticker.load_data()
    exec_msg = e.value.args[0]
    ...
    # gainz只有0820一天数据
    ticker = Ticker("gainz", "local", f"{shared_datadir}", starttime=date(
        2021, 1, 1), endtime=date(2021, 8, 20))
    ticker.load_data()
...
def test_ticker_cal_profit(shared_datadir):
    """测试定投结果"""
    aapl = Ticker("AAPL", "local", f"{shared_datadir}", starttime=date(
        2021, 7, 1), endtime=date(2021, 8, 20))
    aapl.load_data()
    xmm = [
    ...
    assert int(aapl.xmm_profit["profit_percentage"]
               * 1000000) == int(xmm_profit_rate * 1000000)
    assert int(aapl.dmm_profit["profit_percentage"]
               * 1000000) == int(dmm_profit_rate * 1000000)
...
def test_get_today_price_msg(aapl, goev, ogn, shared_datadir):
    ...
    aapl = Ticker("AAPL", "local",
                  f"{shared_datadir}", endtime=date(2021, 8, 18))
    msg = aapl.get_today_price_msg()
...
def test_gen_mmt_msg(aapl, goev):
    ...
    aapl.endtime = date(2021, 8, 20)
    aapl.reload_target_data()
    aapl.cal_profit()
